...
public abstract class ClientRegistryTagDataProvider<T> extends NeoBaseClientTagDataProvider<T, KeyResolver.RegistryWrapper<T>> {
        ...
        return createContentProvider().thenApply(provider -> {
            ...
            return CompletableFuture.allOf(builders.entrySet().stream().map(entry -> {
                ...
                if (!errors.isEmpty()) {
                    ...
                    DataResult<JsonElement> jsonResult = ClientTagFile.codec(keyResolver).encodeStart(JsonOps.INSTANCE, tagFile);
