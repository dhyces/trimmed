...
abstract class BaseTagHandler<K, V> {
    ...
    protected final <KEY, VAL> DataResult<Set<VAL>> resolveTag(Map<ResourceLocation, Set<TagEntry>> unresolvedTags, Map<KEY, Set<VAL>> resolvedTags, ResourceLocation tagId, Function<ResourceLocation, KEY> keyFactory, Function<OptionalId, VAL> valueFactory, LinkedHashSet<ResourceLocation> resolutionSet) {
        ...
        for (TagEntry entry : entries) {
            TagEntryAccessor accessor = (TagEntryAccessor)entry;
            DataResult<Set<VAL>> entryResult = resolveTagEntry(unresolvedTags, resolvedTags, accessor, keyFactory, valueFactory, resolutionSet);
    ...
    protected <KEY, VAL> DataResult<Set<VAL>> resolveTagEntry(Map<ResourceLocation, Set<TagEntry>> unresolvedTags, Map<KEY, Set<VAL>> resolvedTags, TagEntryAccessor tagEntry, Function<ResourceLocation, KEY> keyFactory, Function<OptionalId, VAL> valueFactory, LinkedHashSet<ResourceLocation> resolutionSet) {
        if (tagEntry.isTag()) {
