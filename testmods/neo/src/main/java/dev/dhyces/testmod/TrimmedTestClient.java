...
public class TrimmedTestClient {
    ...
    private void loggedIn(final PlayerEvent.PlayerLoggedInEvent event) {
        ...
        TrimmedClientTagApi.getInstance().getTag(TestClientTags.TEST_BIOME_TAG).getSet().forEach(biomeHolder -> TrimmedTest.LOGGER.info(biomeHolder.toString()));

        TrimmedClientMapApi.getInstance().getSimpleMap(TestClientMapKeys.MANUAL_TEST_MAP).getMap().forEach(TrimmedTestClient::logEntry);
        ...
        TrimmedClientMapApi.getInstance().getSimpleMap(TestClientMapKeys.MANUAL_TEST_BIOME_MAP).getMap().forEach(TrimmedTestClient::logEntry);

        TrimmedTest.TEST_DELEGATE.getMap().forEach(TrimmedTestClient::logEntry);

        TrimmedTest.TEST_DELEGATE_2.getMap().forEach(TrimmedTestClient::logEntry);
    }
    ...
    private static void logEntry(Object key, Object value) {
        TrimmedTest.LOGGER.info("KEY: {}, VALUE: {}", key, value);
    }
